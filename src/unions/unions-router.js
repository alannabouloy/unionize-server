...
unionRouter.get("/", jsonParser, async (req, res, next) => {
...
unionRouter.get("/industry", jsonParser, async (req, res, next) => {
  const db = req.app.get("db");
  const page = req.query.page;
  const search = req.query.q ? req.query.q : "";
  ...
  try {
    let unions = await UnionService.getPaginatedUnionsByIndustry(
      db,
      hasIndustry.id,
      page,
      search
    );
    let count = await UnionService.countUnionsByIndustry(
      db,
      hasIndustry.id,
      search
    );
...
unionRouter.get("/:unionId", async (req, res, next) => {
  const db = req.app.get("db");
